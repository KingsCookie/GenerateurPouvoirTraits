...
- La **moyenne** est arrondie à l'entier le plus proche ; en cas d'égalité exacte (partie décimale `0,5`), on arrondit vers l'entier **pair** le plus proche (arrondi « demi vers le pair », dit *bancaire*). Exemples : `4,5 → 4` ; `5,5 → 6` ; `9,5 → 10` ; `0,5 → 0`. Ce choix est **symétrique** : il évite le biais vers le haut d'un arrondi « demi toujours vers le haut » (qui rendait les valeurs juste au-dessus du barème, ex. 11, bien plus fréquentes que celles juste en dessous, ex. 0).
