...
**Pouvoirs multiples par branche (nouveau, cible v0.15.0).** Certaines feuilles de l'arbre produisent désormais **deux pouvoirs**, notés `"X" ; "Y"` (deux chaînes séparées par `;`). La sous-liste produit alors ces **deux pouvoirs distincts**, chacun recevant ses **propres** valeurs de puissance/maîtrise (§7.2). Lorsqu'une branche à deux pouvoirs comporte le **même** jeton `Kx` (p. ex. `{Kp}`) dans ses deux chaînes, celui-ci désigne le **même** trait généré : la tentative de génération `K` correspondante n'est faite **qu'une seule fois** et son résultat est **réutilisé** dans les deux pouvoirs (p. ex. un `Ka` donnant « contrôle » vaut « contrôle » dans les deux pouvoirs). Si cette tentative `K` échoue, **tout pouvoir de la branche référençant ce jeton** n'est pas produit (comme au cas mono-pouvoir ci-dessus) ; un pouvoir ne référençant **aucun** jeton `K` échoué est **quand même** produit.

> **Note** — Arbre **révisé par l'auteur (cible v0.15.0)** : plusieurs feuilles produisent désormais **deux pouvoirs** (voir ci-dessus) et certaines formulations ont été ajustées. Les comportements pouvant sembler « incohérents » (état non repris dans certaines branches `a & e & ¬p`, `{Ka}`/`{Kp}` absents de certaines feuilles, etc.) restent **volontaires**.
	...
	if e :
		if p :
			if r :
				if aj :
					if et :
						pouvoir = "{a} {e} avec {aj} {et} sur {r} à la place de {p}" ; "{aj} {et} sur {r} à la place de {p}"
					else :
						pouvoir = "{a} {e} avec {aj} sur {r} à la place de {p}" ; "{aj} sur {r} à la place de {p}"
				else :
					if et :
						pouvoir = "{a} {e} avec {r} {et} à la place de {p}" ; "{r} {et} à la place de {p}"
					else :
						pouvoir = "{a} {e} avec {r} à la place de {p}" ; "{r} à la place de {p}"
			else :
				if aj :
					if et :
						pouvoir = "{a} {e} avec {aj} {et} sur {p}" ; "{aj} {et} sur {p}"
					else :
						pouvoir = "{a} {e} avec {aj} sur {p}" ; "{aj} sur {p}"
				else :
					if et :
						pouvoir = "{a} {e} avec {p} {et}" ; "{p} {et}"
					else :
						pouvoir = "{a} {e} avec {p}"
		else:
			if r:
		...
		if aj:
			if et:
				if r:
					if p:
						pouvoir = "{a} {aj} {et} avec {r} à la place de {p}" ; "rends {aj} {et}"
					else:
						pouvoir = "{a} {aj} {et} sur {r}" ; "rends {aj} {et}"
				else:
					if p:
						pouvoir = "{a} {aj} {et} avec {p}" ; "rends {aj} {et}"
					else:
						pouvoir = "{a} {aj} {et}" ; "rends {aj} {et}"
			else:
				if r:
					if p:
			...
			if r:
				if et:
					if p:
						pouvoir = "{a} {r} {et} avec {p}" ; "rends {r} {et}"
					else:
						pouvoir = "{a} {r} {et}" ; "rends {r} {et}"
				else:
					if p:
						pouvoir = "{a} {r} avec {p}"
					else:
				...
				if p:
					if et:
						pouvoir = "{a} {p} {et}" ; "rends {p} {et}"
					else:
						pouvoir = "{a} {p}"
				else:
					if et:
						pouvoir = "{a} {Ke} {et}" ; "rends {Ke} {et}"
					else:
						pouvoir = "{a} {Ke}"
else:
	if p:
		...
		if e:
			if et:
				if aj:
					if r:
						pouvoir = "{Ka} {e} avec {aj} {et} sur {r} à la place de {Kp}" ; "{aj} {et} sur {r} à la place de {Kp}"
					else:
						pouvoir = "{Ka} {e} avec {aj} {et} sur {Kp}" ; "{aj} {et} sur {Kp}"
				else:
					if r:
						pouvoir = "{Ka} {e} avec {r} {et} à la place de {Kp}" ; "{r} {et} à la place de {Kp}"
					else:
						pouvoir = "{Ka} {e} {et}" ; "rends {e} {et}"
			else:
				if aj:
					if r:
						pouvoir = "{Ka} {e} avec {aj} sur {r} à la place de {Kp}" ; "{aj} sur {r} à la place de {Kp}"
					else:
						pouvoir = "{Ka} {e} avec {aj} sur {Kp}" ; "{aj} sur {Kp}"
				else:
					if r:
						pouvoir = "{Ka} {e} avec {r} à la place de {Kp}" ; "{r} à la place de {Kp}"
					else:
						pouvoir = "{Ka} {e}"
		else:
			if aj:
				if et:
					if r:
						pouvoir = "{aj} {et} sur {r} à la place de {Kp}"
					else:
...
#### 6.4.3. Déduplication des pouvoirs quasi identiques d'une personne (par position et traits affichés, cible v0.15.x)

Le nouvel algorithme §6.4.2 peut produire, chez une même personne, des pouvoirs **quasi identiques** : typiquement les **pouvoirs secondaires** (formes réduites « rends … » / « {aj} … sur … ») de deux feuilles proches, dont **les traits affichés de l'un sont inclus dans ceux de l'autre**. Exemples réellement observés : « rends {p} {et} » donnant à la fois `rends os feu et glace` et `rends os feu` ; ou deux pouvoirs secondaires `écorce sur mandibule à la place des os` et `écorce sur mandibule à la place des os et coté droit`. On applique donc, **une fois par personne** et **avant** l'attribution des puissances/maîtrises (§7.2), la déduplication suivante.

**Traits affichés.** Pour chaque pouvoir, on considère l'**ensemble des traits qui apparaissent dans son libellé** — c'est-à-dire les traits des types **effectivement mentionnés dans le gabarit** de ce pouvoir (plus les traits générés `K` qui y figurent), et **non** l'ensemble complet des traits de la sous-liste. C'est essentiel : un pouvoir **secondaire** (p. ex. « {aj} {et} sur {r} à la place de {p} ») **n'affiche pas** le trait principal (l'action) de sa sous-liste ; deux secondaires issus de sous-listes différentes peuvent donc avoir des **traits affichés emboîtés** même si leurs sous-listes complètes diffèrent par leur principal.

**Position.** Chaque feuille numérote ses pouvoirs par **position** : le **1ᵉʳ** pouvoir est **primaire**, le **2ᵉ** (s'il existe) est **secondaire**. On ne compare **que des pouvoirs de même position** — les primaires entre eux, les secondaires entre eux — **quelle que soit la branche**. Un primaire et un secondaire ne sont **jamais** comparés : cela **préserve** les deux pouvoirs d'une même feuille (le secondaire est toujours une forme réduite du primaire, mais on les garde tous deux).

**Règle.** Au sein d'un même groupe de position, on compare les **ensembles de traits affichés** :

- si l'ensemble de traits affichés d'un pouvoir est **inclus** (⊆) dans celui d'un autre pouvoir du même groupe, on **supprime le pouvoir ayant le moins de traits affichés** (on garde le plus « riche ») ;
- en cas d'**égalité** exacte des ensembles de traits affichés, on supprime **le second** dans l'ordre de production ;
- un pouvoir dont l'ensemble de traits affichés n'est **ni inclus ni incluant** vis-à-vis des autres du groupe est **conservé**.

Formulé autrement : dans chaque groupe de position, on ne garde que les pouvoirs dont l'ensemble de traits affichés est **maximal** ; à ensembles **identiques**, seul le **premier** est conservé.

Exemples (p1 et p2, **même position**, traits **affichés**) :

- p1 affiche **tous** les traits de p2 **plus un** ⇒ p2 (moins de traits) est supprimé (`écorce sur mandibule à la place des os` disparaît au profit de `… et coté droit`).
- p1 et p2 affichent **exactement** les mêmes traits ⇒ p2 est supprimé.
- p2 affiche **tous** les traits de p1 **plus un (ou plus)** ⇒ p1 (moins de traits) est supprimé.

Comme les **primaires** affichent toujours leur **trait principal** (action, sinon partie du corps), qui diffère d'une sous-liste à l'autre, ils ne se recouvrent (presque) jamais : en pratique, **ce sont les pouvoirs secondaires qui se dédupliquent**.

**Garde-fou (libellé strictement identique).** En **dernière passe**, après la déduplication par position + traits affichés ci-dessus, si deux pouvoirs restants ont **exactement le même libellé affiché**, on n'en conserve qu'**un seul** : le **premier** dans l'ordre de production. Ce cas ne peut survenir que lorsque le catalogue contient des **traits homonymes** (deux traits distincts partageant le même libellé). Ce garde-fou garantit qu'**aucun libellé n'apparaît deux fois** dans la liste des pouvoirs d'une personne.

Cette étape est purement fonctionnelle : elle **ne compare aucune statistique** (puissances/maîtrises pas encore attribuées), **ne consomme aucun tirage RNG**, **ne modifie pas l'ADN** (les traits générés `K…` déjà inscrits restent actifs), et préserve le **déterminisme**. Les **puissances/maîtrises** (§7.2) sont ensuite attribuées à la **liste dédupliquée**. `FORMAT_VERSION` inchangé.
